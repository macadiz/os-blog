import { Component, Input } from '@angular/core';
import { CommonModule } from '@angular/common';

export interface TagData {
  name: string;
  id?: string;
}

export type TagDisplaySize = 'sm' | 'md' | 'lg';
export type TagDisplayVariant = 'default' | 'minimal';

@Component({
  selector: 'app-tag-display',
  standalone: true,
  imports: [CommonModule],
  template: `
    <span [class]="getTagClasses()">
      <span *ngIf="showIcon" class="material-symbols-outlined mr-1" [class]="getIconSizeClass()">tag</span>
      {{ getDisplayText() }}
    </span>
  `,
})
export class TagDisplayComponent {
  @Input() tag!: TagData;
  @Input() size: TagDisplaySize = 'md';
  @Input() variant: TagDisplayVariant = 'default';
  @Input() showIcon = true;
  @Input() showHash = false;

  getTagClasses(): string {
    const baseClasses = 'inline-flex items-center rounded-lg themed-border border';

    const sizeClasses = {
      sm: 'px-2 py-1 text-xs',
      md: 'px-3 py-1 text-sm',
      lg: 'px-4 py-2 text-base'
    };

    const variantClasses = {
      default: 'bg-theme-accent/10 themed-accent',
      minimal: 'themed-surface-alt themed-text-secondary'
    };

    return `${baseClasses} ${sizeClasses[this.size]} ${variantClasses[this.variant]}`;
  }

  getIconSizeClass(): string {
    const iconSizes = {
      sm: 'text-xs',
      md: 'text-sm',
      lg: 'text-base'
    };
    return iconSizes[this.size];
  }

  getDisplayText(): string {
    return this.showHash ? `#${this.tag.name}` : this.tag.name;
  }
}