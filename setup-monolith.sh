...
# Open Blog Monolith Quick Setup
echo "🏗️  Open Blog Monolith Setup"
echo "============================="
echo ""
...
if ! docker image inspect open-blog:monolith >/dev/null 2>&1; then
    echo "🏗️  Building monolith image (this may take a few minutes)..."
    ./build-monolith.sh
    if [ $? -ne 0 ]; then
        echo "❌ Build failed!"
        exit 1
    ...
    echo "✅ Monolith image already exists"
fi
...
if [[ $START_NOW =~ ^[Yy]$ ]]; then
    echo ""
    echo "🚀 Starting Open Blog Monolith..."
